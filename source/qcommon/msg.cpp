...
void MSG_WriteDeltaEntity( msg_t *msg, const SyncEntityState * from, const SyncEntityState * to, bool force ) {
	...
	Delta( &delta, *const_cast< SyncEntityState * >( to ), *from );
...
void MSG_WriteDeltaPlayerState( msg_t * msg, const SyncPlayerState * from, const SyncPlayerState * to ) {
	...
	Delta( &delta, *const_cast< SyncPlayerState * >( to ), *from );

	MSG_WriteDeltaBuffer( msg, delta );
}

void MSG_ReadDeltaPlayerState( msg_t * msg, const SyncPlayerState * from, SyncPlayerState * to ) {
	static SyncPlayerState dummy;
	if( from == NULL ) {
		from = &dummy;
	}
...
void MSG_WriteDeltaGameState( msg_t * msg, const SyncGameState * from, const SyncGameState * to ) {
	...
	Delta( &delta, *const_cast< SyncGameState * >( to ), *from );
